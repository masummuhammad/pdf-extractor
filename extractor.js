            ...
            */
            ...
            tempArr.forEach(e=>{
                ...
                if(/ihre depotnummer/.test(e[0])){
                    ...
                    let fd=e[0].split(/ +/);
                    let pf=e[1].split(' (');
                    transactionDetails.push({transactionnumber:fd[0],transactiontype:fd[1],underlying:pf[0],isinwkn:pf[1].replace(')','')});
                    
                }
               
                //except tel and email every 2 length array will set it's 1st element to key and 2nd  value
                if(e.length==2 && (e[0].trim()!=="tel." && e[0].trim()!=='e-mail' && e[0].trim()!=='ust-idnr.' && e[0].trim()!=='ihre depotnummer' && e[0].trim()!=='depotinhaber') && !(/blz/.test(e[0])) && !(/nr\.[0-9]+\/[0-9]+/.test(e[0]))){
                ...
                if(/ordervolumen/.test(e[0])){
                    if(obj.hasOwnProperty('ordervolumen')){
                        arrayObj.push(obj);
                        obj={};
                        obj['ordervolumen']=e[1].trim().replace('.','');
                        ...
                        obj['ausgefuehrt']=e[1].trim().replace('.','');
